...
You can specify which agent(s) should work on the task. If no agent is provided, Marvin will use the default agent.
...
You can also assign multiple agents to work together on a task:
...
article = marvin.run(
    "Research and write an article about AI",
