...
In Marvin, agents can be organized into teams to work together on tasks. A team is a way to govern how agents work together. The most common team is a `Swarm`, in which every agent can hand off to every other agent, but other team types exist like round-robin, random, moderated, and more.
...
researcher = marvin.Agent(name="Researcher")
writer = marvin.Agent(name="Writer")
editor = marvin.Agent(name="Editor")

team = Swarm(agents=[researcher, writer, editor])

marvin.run("Write a report about AI", agents=[team])
```
