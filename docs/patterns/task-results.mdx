...
When using Pydantic models, you can use field validators for more complex validation:
...
zip_code = marvin.run(
    ...
    result_type=Annotated[str, "A 5-digit US zip code"]
)

# Get a constrained number
rating = marvin.run(
    "Rate this movie from 1-5",
    result_type=Annotated[int, "A rating between 1 and 5"]
)
``` 