...
import gh_util
import marvin
from gh_util.functions import fetch_contributor_data
    ...
    return JinjaEnvironment(**jinja_settings).from_string(
        inspect.cleandoc(
            ...
            {% for commit in activities.merged_commits %}
                - [{{ commit.message }}]({{ commit.html_url }})
            {% endfor %}
            {% endif %}
            {% endif %}
            {% endfor %}
...
@task(task_run_name="Fetch GitHub Activity in {owner}/{repo} since {since}")
...
async def write_a_tasteful_epic(markdown_digest: str) -> str:
    ...
    proportional to their contribution. Speak specifically in the style if Marvin
    from the Hitchhiker's Guide to the galaxy. Marvin should, with a dampened fondness,
    begrudgingly cheer on the contributors in their efforts, interspersing satirical digs.
    ...
    Do NOT be overly verbose - think step by step like an expert Pythonista to assess impact.
...
@task(retries=3, retry_delay_seconds=3, task_run_name="Draw a movie poster")
@marvin.image
...
async def daily_github_digest(
    ...
    epic_story = await write_a_tasteful_epic(markdown_digest)

    try:
        image_url = draw_a_movie_poster(epic_story).data[0].url
    except BadRequestError as e:
        image_url = f"https://random-d.uk/api/http/{e.status_code}.jpg"
...
if __name__ == "__main__":
    ...
    asyncio.run(
        daily_github_digest(owner="PrefectHQ", repo="marvin", post_story_to_slack=False)
    )
