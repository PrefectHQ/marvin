...
class TestCreateBots:
    async def test_create_bot_with_default_settings(self):
        bot = marvin.Bot()
        assert bot.name == marvin.bots.DEFAULT_NAME
        assert bot.personality == marvin.bots.DEFAULT_PERSONALITY
        assert bot.instructions == marvin.bots.DEFAULT_INSTRUCTIONS
